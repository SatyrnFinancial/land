...
// File: contracts/space/ISPACERegistry.sol
...
// File: contracts/space/SPACERegistry.sol
...
contract SPACERegistry is Storage, Ownable, FullAssetRegistry, ISPACERegistry {
  ...
  function transferSpaceToSector(int x, int y, uint256 sectorId) external {
    require(
      sectorRegistry.ownerOf(sectorId) == msg.sender,
  ...
  */
    ...
    returns (uint256)
  {
    ...
    uint256 sectorTokenId = sectorRegistry.mint(beneficiary, metadata);
    bytes memory sectorTokenIdBytes = toBytes(sectorTokenId);
